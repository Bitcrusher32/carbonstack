...
rehearsal_root="${2:-/tmp/carbonstack-v0.6.0-rehearsal}"
...
echo "===== CarbonStack v0.6.0 package rehearsal ====="
...
"$script_dir/stage-v0.6.0-package.sh" "$stage_root"
...
echo
echo "===== verifying checksums from fresh extraction ====="
cd "$package_root/carbonstack/tools/carbonstack-validate"
go run . --profile verify-checksums --root "$package_root"

echo
echo "===== running full validation from fresh extraction ====="
go run . --profile full --root "$package_root" --clean-generated
...
check_required_file "./carbonstack/registry/COMMAND_BOUNDARY_TABLE.v0.md"
